...
{ 	
	while((PRINTF_BUS->SR&0X40)==0);//循环发送,直到发送完毕   
    PRINTF_BUS->DR = (u8) ch;
	return ch;
}
#endif
...
vu8 USART_RX_BUF[USART_REC_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
void uart_printf_init(u32 bound){
   //GPIO端口设置
    ...
    USART_Init(PRINTF_BUS, &USART_InitStructure); //初始化串口
	
    USART_Cmd(PRINTF_BUS, ENABLE);  //使能串口
	
	//USART_ClearFlag(USART1, USART_FLAG_TC);
	
#if EN_USARTx_RX	
	USART_ITConfig(PRINTF_BUS, USART_IT_RXNE, ENABLE);//开启相关中断
...
//	OSIntEnter();    
//#endif
//	if(USART_GetITStatus(PRINTF_BUS, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
//	{
//		Res =USART_ReceiveData(PRINTF_BUS);//(USARTx->DR);	//读取接收到的数据
//		
